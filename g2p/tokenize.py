...
def read_tsv(path, category=False, remove_tgt_spaces=False):
    # tsv without header
    col_names = ["graphemes", "phonemes"]
    data = {name: [] for name in col_names}
    with open(path, encoding='utf-8') as f:
        for line in f:
            fields = line.rstrip("\n").split("\t")
            for name, field in zip(col_names, fields):
                if remove_tgt_spaces and name == "phonemes":
...
def main(args):
    ...
    # read data
    data = read_tsv(args.corpus, remove_tgt_spaces=True)  # hmm
    src = data["graphemes"]
    tgt = data["phonemes"]
