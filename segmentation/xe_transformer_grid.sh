...
grid() {
    ...
    for WARMUP in 4000 8000 ; do
        for DROPOUT in 0.1 0.3 ; do
            for BATCH in $@ ; do
                MODEL_DIR="${GRID_LOC}/${NAME}-xe-minlev-${EMB}-${HID}-${LAYERS}-${HEADS}-${BATCH}-${LR}-${WARMUP}-${DROPOUT}"
                if [ ! -f "${MODEL_DIR}/dev-5.results" ]
                then
                    bash fairseq_train_xe_transformer.sh $DATA_BIN $NAME $EMB $HID $LAYERS $HEADS $BATCH $LR $WARMUP $DROPOUT $GRID_LOC
